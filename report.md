...
- [C语言代码规范](#c语言代码规范)
    ...
    - [表达式和基本语句](#表达式和基本语句)
        - [运算符的优先级](#运算符的优先级)
                    - [运算符的优先级与结合律](#运算符的优先级与结合律)
        - [复合表达式](#复合表达式)
        - [if 语句](#if-语句)
- [define            MAX   100     /*  C语言的宏常量  */](#define   -       -max  -100    -- c语言的宏常量- )
- [define RADIUS 100](#define-radius-100)
- [define DIAMETER RADIUS * 2](#define-diameter-radius--2)
...
版权和版本的声明位于头文件和定义文件的开头（参见[版权和版本的声明](#版权和版本的声明)），主要内容包括：
...
- 在每个结构体声明之后、每个函数定义结束之后都要加空行。参见[函数之间的空行](#函数之间的空行)
...
- 关键字之后要留空格。象const、virtual、inline、case
...
- 每一个嵌套的函数块，使用一个TAB缩进（TAB设定为4个空格）。
...
- 函数的名字应当使用“动词”或者“动词＋名词”（动宾词组）,第一个词采用全部小写，随后每个单词采用第一个字母大写，其它字母小写方式,
单词之间不用下划线。
...
a = b = c = 0
```
这样的表达式称为复合表达式。复合表达式的优点是：书写简洁，可以提高编译效率。但要防止滥用复合表达式。
...
- 如果循环体内存在逻辑判断，并且循环次数很大，宜将逻辑判断移到循环体的外面。[示例效率低但程序简洁](#示例效率低但程序简洁)
的程序比[示例效率高但程序不简洁](#示例效率高但程序不简洁)多执行了N-1次逻辑判断。并且由于前者老要进行逻辑判断，打断了循环“流水线”作业，使得编译器不能对循环进行优化处理，降低
了效率。如果N非常大，最好采用[示例效率高但程序不简洁](#示例效率高但程序不简洁)的写法，可以提高效率。如果N非常小，两者效率差别并不明显，采用[示例效率低但程序简洁](#示例效率低但程序简洁)的写法比较好，因为程序更加简洁。
...
- 函数体的规模要小，尽量控制在100行代码之内，以便于调试和理解。
...
- 指针操作超越了变量的作用范围。居然没有出错，这可能与编译器有关。
